...
NUNIT_TO_JUNIT_XSLT = """<?xml version="1.0" encoding="UTF-8"?>
...
parser = argparse.ArgumentParser(description="""
	This script is used to transform the NUnit XML report
	generated by Unity into a JUnit XML report that can be
	picked up by the GitLab pipeline
	""")
parser.add_argument("input", metavar="input", 
		    help="The filepath of the NUnit XML report")
parser.add_argument("output", metavar="output",
                   help="The filepath of the JUnit XML report")
args = parser.parse_args()


transform = etree.XSLT(etree.XML(NUNIT_TO_JUNIT_XSLT.encode()))

results = etree.parse(args.input)

converted = transform(results)

converted.write(args.output, pretty_print=True, standalone=True, encoding="utf8")
